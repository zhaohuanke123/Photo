package com.vands.recyclerlisttreeview;
    ...
    final public VH onCreateViewHolder(ViewGroup parent, int viewType) {
        if (expandIcon == null) {
            expandIcon = BitmapFactory.decodeResource(
                    parent.getContext().getResources(), R.drawable.folder);
        }

        if (collapseIcon == null) {
            collapseIcon = BitmapFactory.decodeResource(
                    parent.getContext().getResources(), R.drawable.opened_folder);
        }
        ...
        arrowIcon.getLayoutParams().width = w / 12;
        arrowIcon.getLayoutParams().height = w / 12;
